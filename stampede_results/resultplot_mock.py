...
if __name__ == '__main__':
    ...
    sf, mf = ['imf_dmock_snrx2_nolines_wnewpoly_polydata_1421401518_mcmc',
              'imf_dmock_snrx2_nolines_wnewpoly_polydata_1421401518_model']  #sys.argv[1:]
    ...
    sfig.show()
    # Get the mock info
    for k, v in amock['mock_params'].iteritems():
        model.params[k] = np.atleast_1d(v)
    model.params['spec_norm'] = np.log(model.params['spec_norm'])
    mock_theta = model.theta
    poly = (result['run_params'].get('add_mock_poly', False))
    wiggles = (result['run_params'].get('add_wiggles', False))
    if poly and (wiggles is False):
        const = model.params['normalization_guess'] * model.obs['scale']
        model.params['spec_norm'] = poly[0] - np.log(const)
        model.params['poly_coeffs'] = np.array(poly[1:])
        mock_theta = model.theta
    
    theta_try = [mock_theta, result['chain'][32,-1,:]]
    mwave, mospec, mounc, specvecs = comp_samples(theta_try, model, inlog=True)
    vecs = specvecs[0]
    axes = sfig.axes
    [ax.plot(mwave, v, color='cyan', alpha=0.3)
      for ax,v in zip(axes, vecs[:-1])]

    if wiggles:
        calvec = add_wiggles(datadir='/Users/bjohnson/Projects/cetus/data/')
        wfig = pl.figure()
        wfig.plot(mwave, calvec)
        
    sys.exit()
      
    rfig = pl.figure()
    pl.plot(mwave, specvecs[1][3]/mospec, label = 'sample/obs', alpha=0.5)
    pl.plot(mwave, specvecs[0][3]/mospec, label = 'mock/obs', alpha=0.5)
    pl.plot(mwave, specvecs[0][3]/specvecs[1][3], label = 'mock/sample', alpha=0.5)
    pl.legend(loc =0)

    pfig = phot_figure(result, samples=sample)
    theta_try = [mock_theta, result['chain'][32,-1,:]]
    mwave, mospec, mounc, specvecs = comp_samples(theta_try, model, photflag=1)
    vecs = specvecs[0]
    axes = pfig.axes[0]
    axes.plot(mwave, vecs[0], color='r', alpha=1.0)
